...
for var_code in var_codes:
	heatmap_df_list.append(pd.read_csv(path_stub+"dataframes/"+var_code+var_code+"_"+file_id+"_heatmap.csv", delim_whitespace = True))
for heatmap_df in heatmap_df_list:
# 	print heatmap_df
	zmin_list.append(min(heatmap_df["lowest_inst_v"]))
	zmax_list.append(max(heatmap_df["lowest_inst_v"]))
...
for var_code, heatmap_df in zip(var_codes, heatmap_df_list):
	print "Drawing graph for ("+var_code+")..."
	hm_df = heatmap_df.pivot(var_code+"_0", var_code+"_1", "lowest_inst_v")
	...
	plt.savefig(path_stub+"newgraphs/"+var_code+var_code+"_"+file_id+".png")
	print "Graph saved to "+path_stub+"newgraphs/"+var_code+var_code+"_"+file_id+".png"
